...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(255, 0, 0, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    // Lerp between dark purple and transparent (background color)
    it_anim->second.a = ImLerp(it_anim->second.a, Enabled ? ImColor(64, 0, 64) : ImColor(64, 0, 64, 0), g.IO.DeltaTime * 10.f);

    // Hovered color (lighter purple with transparency)
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(64, 0, 64, 50) : ImColor(64, 0, 64, 0), g.IO.DeltaTime * 10.f);

    // Draw the background with the darker purple color
    ...
    // Always draw the icon in white, regardless of tab state (enabled/disabled)
    PushFont(FrameWork::Assets::FontAwesomeSolid);
    ImVec2 iconPos = total_bb.Min + ImVec2(total_bb.GetWidth() / 2 - CalcTextSize(Icon).x / 2, total_bb.GetHeight() / 2 - CalcTextSize(Icon).y / 2);

    if (Enabled)
    {
        window->DrawList->AddText(iconPos, ImColor(255, 255, 255), Icon); // White for enabled
    }
    else
    {
        //window->DrawList->AddText(iconPos, ImColor(255, 255, 255, 0.4f), Icon); // Slightly transparent white for disabled
        window->DrawList->AddText(iconPos, ImColor(255, 255, 255), Icon); // White for enabled 
    }
    PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    // Change color to a darker purple when the checkbox is checked
    it_anim->second.color = ImLerp(it_anim->second.color, *v ? ImColor(64, 0, 64) : ImColor(14, 14, 14), g.IO.DeltaTime * 10.f);

    // Adjust the border color to a darker shade (you can customize this further if needed)
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(64, 0, 64, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);

    const ImRect check_bb(total_bb.Max - ImVec2(22, 22), total_bb.Max);

    // Set the fill color for the checkbox background to the darker purple
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), GetColorU32(it_anim->second.color), 5);

    // Set the border color for the checkbox
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImGui::GetColorU32(it_anim->second.b), 5);

    // Set the checkmark color (keep it white for contrast)
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    if (*v)
    {
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items_count)
{
    ...
    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13.f), 3.f + it_anim->second.active_circle_anim, ImColor(255, 0, 0));
...
bool ImGui::Combo(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ...
    // Push style colors for the combo box to make the arrow and selection darker purple


    if (!BeginCombo(label, preview_value, ImGuiComboFlags_None))
    {
        PopStyleColor(5);  // Pop the 5 style colors that were pushed earlier
        return false;
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Set dark purple background for the slider
    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(54, 0, 54), 5); // Dark purple
    ...
    if (value > 1)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 1), frame_bb.Min + ImVec2(it_anim->second.a, grab_bb.Max.y - grab_bb.Min.y + 3), ImColor(128, 0, 128), rounding); // Darker purple grab
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, hovered ? ImColor(255, 0, 0, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        it_anim->second.a = ImLerp(it_anim->second.a, hovered ? ImColor(255, 0, 0, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
