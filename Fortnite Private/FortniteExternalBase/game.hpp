...
{
	if (cr3 != handler::fetch_cr3())
	{
		cr3 = handler::fetch_cr3();
	}
}
...
{
	ImGui::GetBackgroundDrawList()->AddRect(ImVec2(x, y), ImVec2(x + w, y + h), ESPColor, 0, 2, settings::aimbot::BOXthickness);
	ImGui::GetBackgroundDrawList()->AddRect(ImVec2(x, y), ImVec2(x + w, y + h), ESPColor, 0, 0.50f, settings::aimbot::BOXthickness);
}

static std::string GetPlayerName(uintptr_t playerState) {
	auto Name = read<uintptr_t>(playerState + 0xAF8);
...
void draw_cornered_box(int x, int y, int w, int h, ImColor ESPColor)
{
	...
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y), ImVec2(x, y + (h / 3)), ESPColor, 0.50);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y), ImVec2(x + (w / 3), y), ESPColor, 0.50);

	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w - (w / 3), y), ImVec2(x + w, y), ESPColor, 0.50);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w, y), ImVec2(x + w, y + (h / 3)), ESPColor, 0.50);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y + h - (h / 3)), ImVec2(x, y + h), ESPColor, 0.50);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y + h), ImVec2(x + (w / 3), y + h), ESPColor, 0.50);

	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w - (w / 3), y + h), ImVec2(x + w, y + h), ESPColor, 0.50);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w, y + h - (h / 3)), ImVec2(x + w, y + h), ESPColor, 0.50);
}

void draw_bounding_cornered_box(const Vector3& position, const Vector3& size, float thickness, ImColor ESPColor)
{
	float x = position.x;
	float y = position.y;
	float w = size.x;
	...
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y), ImVec2(x, y + (h / 3)), ESPColor, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y), ImVec2(x + (w / 3), y), ESPColor, thickness);

	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w - (w / 3), y), ImVec2(x + w, y), ESPColor, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w, y), ImVec2(x + w, y + (h / 3)), ESPColor, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y + h - (h / 3)), ImVec2(x, y + h), ESPColor, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y + h), ImVec2(x + (w / 3), y + h), ESPColor, thickness);

	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w - (w / 3), y + h), ImVec2(x + w, y + h), ESPColor, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w, y + h - (h / 3)), ImVec2(x + w, y + h), ESPColor, thickness);
}

void draw_bounding_box(const Vector3& position, const Vector3& size, float thickness, ImColor ESPColor)
{
	...
	ImGui::GetBackgroundDrawList()->AddRect(ImVec2(position.x, position.y), ImVec2(size.x + position.x, size.y + position.y), ESPColor, 0.0f, 0, thickness);
}
...
//	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(settings::width / 2, settings::height), ImVec2(location.x, location.y), ESPColor, 0.2f);
//}
...
void draw_player_name(Vector2 location, std::string player_name, ImColor ESPColor)
{
	location.y -= 20;
	...
	ImGui::GetBackgroundDrawList()->AddText(ImVec2(location.x - ImGui::CalcTextSize(player_name.c_str()).x / 2, location.y), ESPColor, player_name.c_str());
}
...
//		ImGui::GetBackgroundDrawList()->AddText(ImVec2(location.x - ImGui::CalcTextSize(player_platform.c_str()).x / 2 + 1, location.y + 1), ImGui::ColorConvertFloat4ToU32(ImColor(0, 0, 0)), player_platform.c_str());
//
//		ImGui::GetBackgroundDrawList()->AddText(ImVec2(location.x - ImGui::CalcTextSize(player_platform.c_str()).x / 2, location.y), ESPColor, player_platform.c_str());
//	}
//	else
//	{
//		location.y -= 50;
...
//	ImGui::GetBackgroundDrawList()->AddText(ImVec2(location.x - ImGui::CalcTextSize(dist).x / 2 + 1, location.y + 1), ImGui::ColorConvertFloat4ToU32(ImColor(0, 0, 0)), dist);
//
//	ImGui::GetBackgroundDrawList()->AddText(ImVec2(location.x - ImGui::CalcTextSize(dist).x / 2, location.y), ESPColor, dist);
//}
...
void skeleDrawLine(Vector2 start, Vector2 end, ImColor color, float thickness)
{
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(start.x, start.y), ImVec2(end.x, end.y), color, thickness);
}
void draw_skeleton(uintptr_t mesh, ImColor Color)
{
	...
	if (settings::visuals::outlines) {
		skeleDrawLine(screenPositions[1], screenPositions[4], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // left shoulder to right shoulder
		skeleDrawLine(screenPositions[1], screenPositions[2], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // left shoulder to left elbow
		skeleDrawLine(screenPositions[2], screenPositions[3], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // left elbow to left hand
		skeleDrawLine(screenPositions[4], screenPositions[5], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // right shoulder to right elbow
		skeleDrawLine(screenPositions[5], screenPositions[6], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // right elbow to right hand
		skeleDrawLine(screenPositions[0], screenPositions[7], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // neck to pelvis
		skeleDrawLine(screenPositions[7], screenPositions[8], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // pelvis to left hip
		skeleDrawLine(screenPositions[8], screenPositions[9], ImColor(0, 0, 0), settings::aimbot::OUTthickness);   // left hip to left knee
		skeleDrawLine(screenPositions[9], screenPositions[12], ImColor(0, 0, 0), settings::aimbot::OUTthickness);  // left knee to left foot
		skeleDrawLine(screenPositions[7], screenPositions[10], ImColor(0, 0, 0), settings::aimbot::OUTthickness);  // pelvis to right hip
		skeleDrawLine(screenPositions[10], screenPositions[11], ImColor(0, 0, 0), settings::aimbot::OUTthickness); // right hip to right knee
		skeleDrawLine(screenPositions[11], screenPositions[13], ImColor(0, 0, 0), settings::aimbot::OUTthickness); // right knee to right foot
	}
	skeleDrawLine(screenPositions[1], screenPositions[4], Color, settings::aimbot::thickness);   // left shoulder to right shoulder
	skeleDrawLine(screenPositions[1], screenPositions[2], Color, settings::aimbot::thickness);   // left shoulder to left elbow
	skeleDrawLine(screenPositions[2], screenPositions[3], Color, settings::aimbot::thickness);   // left elbow to left hand
	skeleDrawLine(screenPositions[4], screenPositions[5], Color, settings::aimbot::thickness);   // right shoulder to right elbow
	skeleDrawLine(screenPositions[5], screenPositions[6], Color, settings::aimbot::thickness);   // right elbow to right hand
	skeleDrawLine(screenPositions[0], screenPositions[7], Color, settings::aimbot::thickness);   // neck to pelvis
	skeleDrawLine(screenPositions[7], screenPositions[8], Color, settings::aimbot::thickness);   // pelvis to left hip
	skeleDrawLine(screenPositions[8], screenPositions[9], Color, settings::aimbot::thickness);   // left hip to left knee
	skeleDrawLine(screenPositions[9], screenPositions[12], Color, settings::aimbot::thickness);  // left knee to left foot
	skeleDrawLine(screenPositions[7], screenPositions[10], Color, settings::aimbot::thickness);  // pelvis to right hip
	skeleDrawLine(screenPositions[10], screenPositions[11], Color, settings::aimbot::thickness); // right hip to right knee
	skeleDrawLine(screenPositions[11], screenPositions[13], Color, settings::aimbot::thickness); // right knee to right foot
}
...
bool get_bounding_box(uintptr_t mesh, box_t& box) {
	fbox_sphere_bounds bounds = read<fbox_sphere_bounds>(mesh + 0x100);
...
void game_loop()
{
	...
	for (int i = 0; i < cache::player_count; i++)
	{
		...
		{
			ESPColor = settings::visuals::VisColor;
		}
		else
		{
			ESPColor = settings::visuals::InVisColor;
		}
		...
		if (settings::visuals::enable)
		{
			if (settings::visuals::box)
			{
				...
				{
				case 0:
					draw_bounding_box(bounds.max, bounds.size, settings::aimbot::BOXthickness, ESPColor);
					break;
				case 1:
					draw_bounding_cornered_box(bounds.max, bounds.size, settings::aimbot::BOXthickness, ESPColor);
					break;
				case 2:
					draw_regular_box(head2d.x - (box_width / 2), head2d.y, box_width, box_height, ESPColor);
					break;
				case 3:
					draw_cornered_box(head2d.x - (box_width / 2), head2d.y, box_width, box_height, ESPColor);
					break;
				case 4:
			...
			if (settings::visuals::username)
			{
				draw_player_name(head2d, GetPlayerName(player_state), ESPColor);
